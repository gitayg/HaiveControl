...
type AnalysisStore = Mutex<HashMap<String, (std::collections::BTreeMap<String, (String, std::time::SystemTime)>, std::time::SystemTime)>>;
...
fn recv_analysis(req: &mut Request) -> Resp {
    ...
    let entry = store.entry(key).or_insert_with(|| (std::collections::BTreeMap::new(), std::time::SystemTime::now()));
    if let Some(sections) = v.get("sections").and_then(|x| x.as_object()) {
        for (k, val) in sections {
            if let Some(s) = val.as_str() {
                entry.0.insert(k.clone(), (s.to_string(), std::time::SystemTime::now()));
            }
        }
    }
...
fn analysis_compliance(sections: &std::collections::BTreeMap<String, (String, std::time::SystemTime)>) -> serde_json::Value {
    ...
    match store.get(&key) {
        ...
        Some((sections, updated)) => {
            let secs: Vec<serde_json::Value> = sections
                .iter()
                .map(|(k, (out, at))| serde_json::json!({"kind": k, "output": out, "secs_ago": now.duration_since(*at).unwrap_or_default().as_secs()}))
                .collect();
            json_resp(&serde_json::json!({
                "ok": true,
                "sections": secs,
                "compliance": analysis_compliance(sections),
                "updated_secs": now.duration_since(*updated).unwrap_or_default().as_secs()
            }))
        }
    }
}
...
fn register(req: &mut Request, agents: &Agents) {
    ...
    if let Ok(mut v) = serde_json::from_str::<serde_json::Value>(&body) {
        ...
        agents.lock().unwrap().insert(ip, Agent { data: v, last: std::time::SystemTime::now() });
    }
}
            ...
            if let Some(o) = d.as_object_mut() {
                ...
                if let Some(events) = alog.get(key) {
                    ...
                    let recent: Vec<serde_json::Value> = events
                        .iter()
                        .map(|(at, act, own, det)| (now_i.duration_since(*at).as_secs(), act, own, det))
                        .filter(|(secs, ..)| *secs < 300)
                        .map(|(secs, act, own, det)| serde_json::json!({"action": act, "owner": own, "secs": secs, "detail": det}))
                        .collect();
                    let active = events.front().map(|(at, ..)| now_i.duration_since(*at).as_secs() < 10).unwrap_or(false);
...
.ov-summary .uncl-sum{color:#ffc98a;cursor:pointer;font-weight:600}
.own-chip{display:inline-block;font-size:8px;font-weight:700;letter-spacing:.03em;padding:1px 5px;border-radius:4px;margin-left:6px;vertical-align:middle}
.own-chip.unclaimed{background:#5a3a20;color:#ffc98a}
...
.agi-btn{padding:4px 9px;font-size:11px;white-space:nowrap}
.ov-refresh{float:right;display:inline-flex;align-items:center;gap:6px;padding:5px 11px;font-size:12px}
.ic-rf{width:14px;height:14px}
.ov-refresh.spin .ic-rf{animation:rfspin .7s linear}
...
.imchip{display:inline-block;font-size:8px;font-weight:700;letter-spacing:.03em;padding:1px 5px;border-radius:4px;margin-left:6px;vertical-align:middle;text-transform:uppercase}
.imchip.im-svc{background:#1e3a5a;color:#8ac4ff}
.imchip.im-auto{background:#25432a;color:#9affb0}
.imchip.im-eph{background:#3a3320;color:#ffd59a}
.lockchip{font-size:10px;vertical-align:middle;opacity:.85}
.pres{display:inline-block;width:7px;height:7px;border-radius:50%;margin-right:5px;vertical-align:middle;background:#555}
.pres.on{background:var(--on);box-shadow:0 0 5px var(--on)}
.pres.idle{background:var(--idle)}
.pres.off{background:#555}
...
.railsearch{width:100%;margin:0 0 8px;background:var(--surface2);border:1px solid var(--line2);color:var(--text);border-radius:8px;padding:6px 9px;font-size:12px;box-sizing:border-box}
.railsearch:focus{outline:none;border-color:var(--accent)}
.rail .navb{margin:0}
...
function durTxt(s){if(s==null)return '';return s<60?(s+'s'):(s<3600?(Math.floor(s/60)+'m'):(Math.floor(s/3600)+'h'));}
...
function ownerChip(d){return d.owner?'':' <span class="own-chip unclaimed" title="UNCLAIMED — no owner, so every user on this hub can see this device. Claim it to make it private to you.">⚠ unclaimed</span>';}
...
L.push('#   run a command:');
L.push('curl -s -X POST \"'+hub+'/m/exec?mtok='+tok+oq+'\" -H \"Content-Type: application/json\" -d '+'\x27'+'{\"target\":\"'+base+'\",\"cmd\":\"whoami\"}'+'\x27');
L.push('#   launch a GUI app (fire-and-forget, never wedges the channel):');
L.push('curl -s -X POST \"'+hub+'/m/exec?mtok='+tok+oq+'\" -H \"Content-Type: application/json\" -d '+'\x27'+'{\"target\":\"'+base+'\",\"cmd\":\"notepad\",\"detach\":true}'+'\x27');
L.push('#   system report / action (kind=hardware|packages|services|reboot|posture|firewall_on|install …):');
L.push('curl -s \"'+hub+'/m/sys?mtok='+tok+oq+'&target='+base+'&kind=hardware\"');
L.push('');
L.push('# 3) every command the agent supports (machine-readable catalog + how to invoke)');
L.push('curl -s \"'+hub+'/m/actions?mtok='+tok+oq+'\"');
...
function doPosture(){out('checking compliance…');fetch(API+'/x/sys?target='+enc(SEL)+'&kind=posture').then(function(r){return r.json();}).then(function(j){if(!j.ok){out('[error] '+(j.error||'failed'));return;}var s='Compliance: '+j.grade+' ('+j.score+'/100)\n';(j.checks||[]).forEach(function(c){s+='  ['+(c.pass?'PASS':'FAIL')+'] '+c.check+'\n';});out(s);}).catch(function(e){out('error: '+e);});}
...
function loadSchedules(){fetch(API+'/x/schedules').then(function(r){return r.json();}).then(function(j){renderSchedules((j&&j.schedules)||[]);}).catch(function(){document.getElementById('sched-list').innerHTML='<div class="aud-empty">Could not load schedules.</div>';});}
...
function delSchedule(id){fetch(API+'/x/schedule-delete?id='+encodeURIComponent(id)).then(function(r){return r.json();}).then(function(){loadSchedules();}).catch(function(){});}
...
function loadRecordings(){fetch(API+'/x/recordings').then(function(r){return r.json();}).then(function(j){renderRecordings((j&&j.recordings)||[]);}).catch(function(){document.getElementById('rec-list').innerHTML='<div class="aud-empty">Could not load recordings.</div>';});}
...
function playRecording(file,dev){stopPlay();document.getElementById('rec-player').style.display='block';document.getElementById('rec-title').textContent='▶ '+dev+' — replaying…';fetch(API+'/x/recording?file='+encodeURIComponent(file)).then(function(r){return r.text();}).then(function(txt){var lines=txt.split('\n');var term=new Terminal({convertEol:false,fontSize:12,cols:120,rows:30,theme:{background:'#0d0f14',foreground:'#e6e9f2'}});term.open(document.getElementById('rec-term'));REC_TIMERS=[];var last=0;for(var i=1;i<lines.length;i++){if(!lines[i])continue;try{var e=JSON.parse(lines[i]);if(e[1]!=='o')continue;(function(data,at){REC_TIMERS.push(setTimeout(function(){term.write(data);},at*1000));})(e[2],e[0]);if(e[0]>last)last=e[0];}catch(x){}}REC_TIMERS.push(setTimeout(function(){document.getElementById('rec-title').textContent='▶ '+dev+' — done';},last*1000+300));}).catch(function(){document.getElementById('rec-title').textContent='playback failed';});}
function delRecording(file){if(!confirm('Delete this recording?'))return;fetch(API+'/x/recording-delete?file='+encodeURIComponent(file)).then(function(r){return r.json();}).then(function(){loadRecordings();}).catch(function(){});}
var MAP_ON=false,LMAP=null,INV_MODE=null,DASH_ON=false,BOOTED=false;
function onSearch(v){SEARCH=(v||'').toLowerCase();renderSide(LAST);if(SEARCH){INV_MODE='table';if(document.getElementById('overview-view').style.display!=='none'){renderOverview(LAST);}else{showInventory();}}else if(document.getElementById('overview-view').style.display!=='none'){renderOverview(LAST);}}
...
function ensureLeaflet(cb){if(window.L||window.__lfTried){cb();return;}window.__lfTried=true;var css=document.createElement('link');css.rel='stylesheet';css.href=API+'/bin/leaflet.css';document.head.appendChild(css);var s=document.createElement('script');s.src=API+'/bin/leaflet.js';s.onload=cb;s.onerror=cb;document.head.appendChild(s);}
...
function loadMap(){fetch(API+'/x/geo').then(function(r){return r.json();}).then(function(j){renderMap((j&&j.devices)||[]);}).catch(function(){document.getElementById('map-svg').innerHTML='<div class="aud-empty">Could not load map.</div>';});}
...
function loadSettings(){fetch(API+'/x/settings').then(function(r){return r.json();}).then(function(j){document.getElementById('set-au').value=j.agent_update||'manual';document.getElementById('set-tray').value=j.tray||'on';document.getElementById('set-ver').textContent='serving v'+(j.server_version||'');}).catch(function(){});}
function saveSetting(key,el){var body={};body[key]=document.getElementById(el).value;fetch(API+'/x/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json();}).then(function(){}).catch(function(){});}
...
function submitScript(){var name=document.getElementById('sf-name').value.trim();var bodyv=document.getElementById('sf-body').value;var msg=document.getElementById('sf-msg');if(!name||!bodyv.trim()){msg.textContent='name and script body are required';return;}msg.textContent='saving…';fetch(API+'/x/script-add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,description:document.getElementById('sf-desc').value,shell:document.getElementById('sf-shell').value,platforms:document.getElementById('sf-plat').value.split(','),body:bodyv})}).then(function(r){return r.json();}).then(function(j){if(j.ok){document.getElementById('sf-name').value='';document.getElementById('sf-desc').value='';document.getElementById('sf-body').value='';toggleScrForm();searchScripts();}else{msg.textContent=j.error||'failed';}}).catch(function(){msg.textContent='request failed';});}
function deleteScript(ev,file){ev.stopPropagation();if(!confirm('Delete this custom script?'))return;fetch(API+'/x/script-delete?file='+encodeURIComponent(file)).then(function(r){return r.json();}).then(function(){searchScripts();}).catch(function(){});}
...
function specHtml(d){function sp(l,v){return (v!=null&&v!=='')?('<span class="spec"><span class="sl">'+l+'</span><span class="sv">'+esc2(v)+'</span></span>'):'';}var ifs='';(d.interfaces||[]).forEach(function(i){if(!i.addr||i.addr.indexOf('fe80')===0||i.addr==='::1'||i.addr.indexOf('127.')===0)return;ifs+='<span class="chip"><b>'+esc2(i.name)+'</b> '+esc2(i.addr)+'</span>';});var mics='';(d.microphones||[]).forEach(function(m){mics+='<span class="chip mic">🎙 '+esc2(m)+'</span>';});var chips=(ifs||mics)?('<div class="chiprow">'+ifs+mics+'</div>'):'';var av=d.agent_version||'',sv=(window.HB&&(window.HB.agent_ver||window.HB.ver))||'';var agln=av?('v'+av+(sv?(av===sv?' (current)':(' — update to '+sv)):'')):(sv?(sv+' available from hub'):'');var pi=presInfo(d);var loginTxt=(d.logged_in===false)?'no user logged in':((pi.u&&pi.u!=='—')?(pi.u+' · '+(d.locked===true?'🔒 screen locked':(d.active===true?'active':(d.active===false?('idle'+(d.idle_secs!=null?(' '+durTxt(d.idle_secs)):'')):'logged in')))):'');return sp('Hostname',d.hostname)+sp('OS',(d.os||'')+(d.arch?(' ('+d.arch+')'):''))+sp('Install mode',d.install_mode)+sp('User',d.user)+sp('Logged in',loginTxt)+sp('Owner',ownerTxt(d))+sp('CPU',d.cpu)+sp('Cores',d.cores)+sp('Memory',d.mem_gb?(d.mem_gb+' GB total'):'')+sp('Agent',agln)+metersHtml(d)+chips;}
...
function loadPlugins(){fetch(API+'/plugins').then(function(r){return r.json();}).then(function(j){PLUGINS=(j&&j.plugins)||[];var dc=document.getElementById('d-controls');if(dc&&SEL&&DEV[SEL]&&dc.querySelector('.alist'))dc.innerHTML=buildControls(DEV[SEL]);}).catch(function(){});}
